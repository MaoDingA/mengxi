// test_utils.rs — Test-only helpers and tests for the CLI crate.
//
// This file holds:
//   - Pure-test-only helper functions (format_breakdown, low_result_explanation,
//     record_feedback_if_needed, resolve_hybrid_weights, resolve_image_to_file_id)
//   - Duplicate test copies of production helpers (truncate_str, format_duration_ms,
//     seconds_to_datetime) — kept here so existing tests that reference them keep working
//   - The entire #[cfg(test)] mod tests block formerly in main.rs
...
mod tests {
    use super::*;
    use crate::{Cli, Commands, DbSubcommand, extract_command_info, generate_session_id};
    use clap::Parser;
